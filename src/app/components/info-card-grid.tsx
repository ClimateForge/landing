	...
	return (
		...
		>
			<Image
				...
				src={`/landing/card-icons/${icon}.svg`}
				alt="card icon"
				width={40}
				height={40}
			/>
			...
			<p
				className="text-gray-600 font-normal pb-2
...
export default function InfoCardGrid({
	...
	return infoCardsData ? (
		<section
			className={`relative flex flex-col items-center w-full gap-10 ${className} px-4`}
		>
			<h2 className="text-center">
				{title[0]} <GradientText>{title[1]}</GradientText>
			</h2>
			<p className="text-[18px] text-center font-medium max-w-[588px]">
				{description}
			</p>
			<div
